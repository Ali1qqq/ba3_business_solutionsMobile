...
class FilteringDataGrid<T> extends StatefulWidget {
  final void Function(int index, String id, void Function(String) init)
      onCellTap;
...
class _FilteringDataGridState<T> extends State<FilteringDataGrid<T>> {
  ...
  Widget build(BuildContext context) {
    return infoDataGridSource == null
        ? Scaffold(
            body: Center(
        ...
        : Scaffold(
            appBar: AppBar(
              ...
              actions: [
                Text(
                    "عدد العناصر المتأثرة: ${infoDataGridSource!.effectiveRows.isEmpty ? infoDataGridSource!.dataGridRows.length.toString() : infoDataGridSource!.effectiveRows.length.toString()}"),
                const SizedBox(
                  width: 25,
                ),
                ElevatedButton(
                    style: ButtonStyle(
                      backgroundColor:
                          MaterialStatePropertyAll(Colors.lightBlue.shade100),
                ...
                Builder(
                    ...
                    return SfDataGrid(
                      ...
                      onCellTap:
                        ...
                        if (dataGridCellTapDetails.rowColumnIndex.rowIndex -
                                1 >=
                            0) {
                          if (infoDataGridSource!.effectiveRows.isEmpty) {
                            widget.onCellTap(
                                dataGridCellTapDetails.rowColumnIndex.rowIndex -
                                    1,
                                infoDataGridSource!.rows[dataGridCellTapDetails
                            ...
                            widget.onCellTap(
                                dataGridCellTapDetails.rowColumnIndex.rowIndex -
                                    1,
                                infoDataGridSource!.effectiveRows[
                      ...
                      columns: [
                        GridColumn(
                            ...
                            columnName: constructor
                                .affectedKey(type: widget.globalType)
                                .toString(),
                            label: Column(
                              children: [
                                SizedBox(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Text(
                                        constructor
                                                .affectedKey(
                                                    type: widget.globalType)
                                                .toString() +
                                            tab,
                                        textDirection: isArabic.hasMatch(
                                                constructor
                                                    .affectedKey(
                                                        type: widget.globalType)
                                                    .toString())
                        ...
                        ...(widget.globalType != null
                                ? constructor
                                    .toMap(type: widget.globalType)
                                    .entries
                                : constructor.toMap().entries)
                            .map((e) => GridColumn(
                                // width: double.nan,
                                columnWidthMode: ColumnWidthMode.fill,
  ...
  void rebuild(dart) async {
    Map<String, List<FilterCondition>> oldFilter =
        infoDataGridSource?.filterConditions ?? {};
...
void exportData(InfoDataGridSource infoDataGridSource) async {
  ...
  final allData = await compute<({List<DataGridRow> a, DataGridRow b}), String>(
      (message) {
    String _ = "";
    List<List> data = [];
    data = (message.a
        .map((e) => e.getCells().map((e) => e.value).toList())
        .toList());
    String header =
        message.b.getCells().map((e) => e.columnName).toList().join(tab) + "\n";
    _ = header;
    _ = _ + data.map((e) => e.map((e) => e.toString()).join(tab)).join("\n");
    return _;
  }, (
    a: infoDataGridSource.effectiveRows,
    b: infoDataGridSource.dataGridRows[0]
  ));
          ...
          children: [
            ...
            Expanded(
              child: Row(
                ...
                children: [
                  Text(
                    row.getCells().last.columnName ==
                            row.getCells()[index].columnName
                        ? (row.getCells()[index].value ?? "").toString() + '\n'
                        : (row.getCells()[index].value ?? "").toString() + tab,
                    textDirection: isArabic
                            .hasMatch(row.getCells()[index].value.toString())
                        ? TextDirection.rtl
                        : TextDirection.ltr,
